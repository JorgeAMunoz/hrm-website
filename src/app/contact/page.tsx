...
import { Phone, Mail, MapPin, AlertTriangle, Clock } from 'lucide-react'; // Added Clock icon
import ContactForm from '@/components/contact-form';
import { SiteConfig } from '@/config/site';
import GoogleMapEmbed from '@/components/google-map-embed';

export const metadata: Metadata = {
  title: `Contact High Rise Mechanical | NYC Plumber & Heating`,
  description: `Contact High Rise Mechanical for plumbing and heating services in NYC (Bronx, Manhattan, Brooklyn, Queens, Staten Island). Call ${SiteConfig.phoneNumber} for quotes, service requests, or 24/7 emergency support. Office: ${SiteConfig.address}.`,
  keywords: [...SiteConfig.keywords, "contact High Rise Mechanical", "plumbing quote NYC", "heating estimate NYC", "Bronx plumber contact", "NYC plumber phone number", "Manhattan plumber contact", "Brooklyn heating contact"],
   alternates: {
    canonical: '/contact',
  },
};
...
export default function ContactPage() {
  ...
  return (
    <div className="container mx-auto py-16 px-4 md:px-8 lg:px-16">
      <h1 className="text-4xl font-bold text-center mb-12 text-secondary">Contact High Rise Mechanical</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div>
          ...
          <p className="text-foreground/80 mb-8">
            Have a question, need a quote for a non-emergency service, or want to discuss a project? Fill out the form below, and our team will get back to you promptly during business hours.
          </p>
          <ContactForm />
        </div>
        ...
        <div>
                  ...
                  <p className="text-sm text-foreground/80 mb-1">For urgent issues like burst pipes, gas leaks, or no heat/hot water, call us 24/7:</p>
                   <Button asChild variant="link" size="lg" className="p-0 h-auto mt-1 text-destructive font-bold text-xl hover:underline">
                      <a href={`tel:${SiteConfig.phoneNumber.replace(/\D/g, '')}`}>
                         <Phone className="mr-2 h-5 w-5" /> {SiteConfig.phoneNumber}
                      </a>
                   </Button>
              ...
              {SiteConfig.openingHours && (
                 <div className="flex items-start space-x-3">
                   ...
                   <div>
                     ...
                     <p className="text-foreground/80">Standard Hours: [e.g., Monday - Friday: 8:00 AM - 5:00 PM]</p>
          ...
          <div className="aspect-video w-full rounded-lg overflow-hidden shadow-md border border-border">
              {/* Use the GoogleMapEmbed component - Ensure NEXT_PUBLIC_GOOGLE_MAPS_API_KEY is set */}
              <GoogleMapEmbed address={SiteConfig.address} />
          </div>

           {/* Service Area Reminder */}
           <div className="mt-8 bg-muted/50 p-4 rounded-md border border-border">
