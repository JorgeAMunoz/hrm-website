...
export const metadata: Metadata = {
  ...
  description: `Contact ${SiteConfig.name} for expert plumbing & heating services in NYC (Bronx, Manhattan, Brooklyn, Queens, Staten Island). Call ${SiteConfig.phoneNumber} for quotes, schedule service online, or get 24/7 emergency support. Office: ${SiteConfig.address}.`, // Refined description
  keywords: [...SiteConfig.keywords, "contact High Rise Mechanical", "plumbing quote NYC", "heating estimate NYC", "Bronx plumber contact", "NYC plumber phone number", "Manhattan plumber contact", "Brooklyn heating contact", "schedule plumbing NYC"], // Added schedule keyword
   alternates: {
    canonical: '/contact',
  },
};
        ...
        <div>
          <h2 className="text-2xl font-semibold mb-6 text-secondary">Contact Information</h2>
           ...
           <div className="bg-destructive/10 border-l-4 border-destructive p-4 rounded-md mb-8 shadow-md">
             <div className="flex items-start space-x-3"> {/* Changed to items-start for better alignment on smaller screens */}
                  ...
                  <p className="text-sm text-foreground/80 mb-1">For urgent issues like burst pipes, gas leaks, or no heat/hot water, call us 24/7:</p>
                   <Button asChild variant="link" size="lg" className="p-0 h-auto mt-1 text-destructive font-bold text-xl hover:underline">
                      <a href={`tel:${SiteConfig.phoneNumber.replace(/\D/g, '')}`}>
                         <Phone className="mr-2 h-5 w-5 inline" /> {SiteConfig.phoneNumber} {/* Added inline style */}
                      </a>
                   </Button>
